...
/**
 * Converting from the C Date format to the Java data format is expensive when done for every record at once. I use this
 * class to allow lazy conversions between the formats.
 * 
 * @author bpellin
 */
public class PwDate implements Cloneable {
	...
	/**
	 * Unpack date from 5 byte format. The five bytes at 'offset' are unpacked to a java.util.Date instance.
	 */
	...
	public static byte[] writeTime(Date date, Calendar cal) {
		...
		buf[1] = Types.writeUByte(((year & 0x0000003F) << 2) | ((month >> 2) & 0x00000003));
