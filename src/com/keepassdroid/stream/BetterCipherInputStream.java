...
/**
 * This class wraps an {@code InputStream} and a cipher so that {@code read()} methods return data that are read from
 * the underlying {@code InputStream} and processed by the cipher.
 * <p>
 * The cipher must be initialized for the requested operation before being used by a {@code BetterCipherInputStream}.
 * For example, if a cipher initialized for decryption is used with a {@code BetterCipherInputStream}, the
 * {@code BetterCipherInputStream} tries to read the data an decrypt them before returning.
 */
public class BetterCipherInputStream extends FilterInputStream {
	 ...
	 * @return the number of bytes filled into buffer {@code b}, or {@code -1} if the end of the stream is reached.
	 * @throws IOException
	 *           if an error occurs.
	 */
	@Override
	 ...
	 * @return the number of bytes filled into buffer {@code b}, or {@code -1} of the of the stream is reached.
	 * @throws IOException
	 *           if an error occurs.
	 ...
	 * <p>
	 * The number of bytes skipped depends on the result of a call to {@link BetterCipherInputStream#available()
	 * available}. The smaller of n and the result are the number of bytes being skipped.
	 * 
	 * @param n
	...
	/**
	 * Closes this {@code BetterCipherInputStream}, also closes the underlying input stream and call {@code doFinal} on
	 * the cipher object.
	 * 
	 * @throws IOException
	 *           if an error occurs.
	 ...
	 * 
	 * @return false, since this input stream does not support {@code mark} and {@code reset}.
	 */
	@Override
