...
public abstract class WrapCipherSpi extends CipherSpi implements PBE {
	...
	protected Key engineUnwrap(byte[] wrappedKey, String wrappedKeyAlgorithm, int wrappedKeyType) throws InvalidKeyException {
		 ...
		 * 
		 * // * The caller doesnt know the algorithm as it is part of // * the encrypted data.
		 * 
		 * ASN1InputStream bIn = new ASN1InputStream(encoded); PrivateKey privKey;
		 * 
		 * try { ASN1Sequence s = (ASN1Sequence)bIn.readObject(); PrivateKeyInfo in = new PrivateKeyInfo(s);
		 * 
		 * DERObjectIdentifier oid = in.getAlgorithmId().getObjectId();
		 * 
		 * 
		 * if (oid.equals(X9ObjectIdentifiers.id_ecPublicKey)) { privKey = new JCEECPrivateKey(in); } else if
		 * (oid.equals(CryptoProObjectIdentifiers.gostR3410_94)) { privKey = new JDKGOST3410PrivateKey(in); }
		 * 
		 * else if (oid.equals(X9ObjectIdentifiers.id_dsa)) { privKey = new JDKDSAPrivateKey(in); } else if
		 * (oid.equals(PKCSObjectIdentifiers.dhKeyAgreement)) { privKey = new JCEDHPrivateKey(in); } else if
		 * (oid.equals(X9ObjectIdentifiers.dhpublicnumber)) { privKey = new JCEDHPrivateKey(in); } else // the old standby!
		 * { privKey = new JCERSAPrivateCrtKey(in); } } catch (Exception e) { throw new
	...
	/*
	 * public static class DESEDEWrap extends WrapCipherSpi { public DESEDEWrap() { super(new DESedeWrapEngine()); } }
	 * 
	 * public static class RC2Wrap extends WrapCipherSpi { public RC2Wrap() { super(new RC2WrapEngine()); } }
	 * 
	 * public static class RFC3211DESedeWrap extends WrapCipherSpi { public RFC3211DESedeWrap() { super(new
	 * RFC3211WrapEngine(new DESedeEngine()), 8); } }
	 */
}
