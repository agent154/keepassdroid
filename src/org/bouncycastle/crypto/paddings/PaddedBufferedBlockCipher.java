...
/**
 * A wrapper class that allows block ciphers to be used to process data in a piecemeal fashion with padding. The
 * PaddedBufferedBlockCipher outputs a block only when the buffer is full and more data is being added, or on a doFinal
 * (unless the current block in the buffer is a pad block). The default padding mechanism used is the one outlined in
 * PKCS5/PKCS7.
 */
public class PaddedBufferedBlockCipher extends BufferedBlockCipher {
	...
	/**
	 * Process the last block in the buffer. If the buffer is currently full and padding needs to be added a call to
	 * doFinal will produce 2 * getBlockSize() bytes.
	 * 
	 * @param out
	 ...
	 * @return the number of output bytes copied to out.
	 * @exception DataLengthException
	 *              if there is insufficient space in out for the output or we are decrypting and the input is not block
	 *              size aligned.
	 * @exception IllegalStateException
