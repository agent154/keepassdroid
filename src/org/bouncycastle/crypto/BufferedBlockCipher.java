...
/**
 * A wrapper class that allows block ciphers to be used to process data in a piecemeal fashion. The BufferedBlockCipher
 * outputs a block only when the buffer is full and more data is being added, or on a doFinal.
 * <p>
 * Note: in the case where the underlying cipher is either a CFB cipher or an OFB one the last block may not be a
...
public class BufferedBlockCipher {
	...
	public BufferedBlockCipher(BlockCipher cipher) {
		...
		if (pgpCFB) {
			...
			partialBlockOkay = (idx > 0 && (name.startsWith("CFB", idx) || name.startsWith("OFB", idx) || name.startsWith("OpenPGP", idx) || name.startsWith("SIC", idx) || name.startsWith("GCTR",
					idx)));
		}
	}
	 ...
	 * @param length
	 *          the length of the input.
	 * @return the space required to accommodate a call to update and doFinal with 'length' bytes of input.
	 */
	 ...
	 * @return the number of output bytes copied to out.
	 * @exception DataLengthException
	 *              if there is insufficient space in out for the output, or the input is not block size aligned and
	 *              should be.
	 * @exception IllegalStateException
	...
	/**
	 * Reset the buffer and cipher. After resetting the object is in the same state as it was after the last init (if
	 * there was one).
	 */
	public void reset() {
		//
