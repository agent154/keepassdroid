...
public abstract class ASN1Sequence extends ASN1Object {
	...
	/**
	 * Return an ASN1 sequence from a tagged object. There is a special case here, if an object appears to have been
	 * explicitly tagged on reading but we were expecting it to be implicitly tagged in the normal course of events it
	 * indicates that we lost the surrounding sequence - so we need to add it back (this will happen if the tagged object
	 * is a sequence that contains other sequences). If you are dealing with implicitly tagged sequences you really
