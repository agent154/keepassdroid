 ...
 * <p>
 * This class implements all the features necessary for a subclass reference hash-based map. Key-value entries are
 * stored in instances of the <code>ReferenceEntry</code> class which can be overridden and replaced. The iterators can
 * similarly be replaced, without the need to replace the KeySet, EntrySet and Values view classes.
 * <p>
 ...
 * <p>
 * When you construct an <code>AbstractReferenceMap</code>, you can specify what kind of references are used to store
 * the map's keys and values. If non-hard references are used, then the garbage collector can remove mappings if a key
 * or value becomes unreachable, or if the JVM's memory is running low. For information on how the different reference
 * types behave, see {@link Reference}.
 * <p>
 * Different types of references can be specified for keys and values. The keys can be configured to be weak but the
 * values hard, in which case this class will behave like a <a
 * href="http://java.sun.com/j2se/1.4/docs/api/java/util/WeakHashMap.html"> <code>WeakHashMap</code></a>. However, you
 * can also specify hard keys and weak values, or any other combination. The default constructor uses hard keys and soft
 * values, providing a memory-sensitive cache.
 * <p>
 * This {@link Map} implementation does <i>not</i> allow null elements. Attempting to add a null key or value to the map
 * will raise a <code>NullPointerException</code>.
 * <p>
 * All the available iterators can be reset back to the start by casting to <code>ResettableIterator</code> and calling
 * <code>reset()</code>.
 * <p>
 * This implementation is not synchronized. You can use {@link java.util.Collections#synchronizedMap} to provide
 * synchronized access to a <code>ReferenceMap</code>.
 * 
...
public abstract class AbstractReferenceMap extends AbstractHashedMap {
	 ...
	 * 
	 * @param keyType
	 *          the type of reference to use for keys; must be {@link #HARD}, {@link #SOFT}, {@link #WEAK}
	 * @param valueType
	 *          the type of reference to use for values; must be {@link #HARD}, {@link #SOFT}, {@link #WEAK}
	 * @param capacity
	...
	/**
	 * Returns a set view of this map's entries. An iterator returned entry is valid until <code>next()</code> is called
	 * again. The <code>setValue()</code> method on the <code>toArray</code> entries has no effect.
	 * 
	 ...
	 * <p>
	 * Note that this method is not synchronized! Special care must be taken if, for instance, you want stale mappings to
	 * be removed on a periodic basis by some background thread.
	 */
	...
	protected static class ReferenceEntry extends HashEntry {
		 ...
		 * <p>
		 * This implementation uses <code>isEqualKey</code> and <code>isEqualValue</code> on the main map for comparison.
		 * 
		 * @param obj
		 ...
		 * @param hash
		 *          the hash code of the <i>key</i> of the mapping; this number might be different from referent.hashCode()
		 *          if the referent represents a value and not a key
		 */
